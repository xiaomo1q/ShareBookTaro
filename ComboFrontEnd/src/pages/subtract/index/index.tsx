...
import { Add_only_square_fav } from '@/service/index';
      ...
      <View className={`flex-col ${styles['si-list']}`}>
        {get_list && get_list.length > 0 && get_list.map((item, i) => (
          <View className={`flex-col ${styles['space-y-9']} ${styles['list-item']}`} key={i} >
            <View className='flex-col' onClick={() => { Taro.redirectTo({ url: '/pagesA/subtract/only-detail/index?_id=' + item.id }) }}>
            ...
            <View className={`flex-row ${styles['space-x-59-reverse']}`}>
              <View className={`flex-row ${styles['space-x-num']}`} onClick={() => { Taro.redirectTo({ url: '/pagesA/subtract/only-detail/index?_id=' + item.id }) }}>
                <Image src={process.env.URL + 'icon/pinglun.svg'} className={`${styles['image_4']}`} />
                <Text className={`${styles['text_8']}`}>{item?.comment_num}</Text>
              </View>
              <View className={`flex-row ${styles['space-x-num']}`} onClick={() => onlyFavClickedHandler(item)}>

...
import { Table, Button, Modal, Form, Input, Space, Select, Card, message } from 'antd';
import { accountUserList, accountUserRole, accountUserDel } from '@/services/user'
...
const Home: React.FC<any> = (props) => {
    ...
    // 获取
    const fetchTable = () => {
        accountUserList().then(res => { setData(res) })
    }
    ...
    const editRowClickedHandler = (row: any) => {
        editForm.setFieldsValue({ username: row.username, id: row._id, role: row.role, dsc: row.dsc })
    ...
    return (
        <div>
            <Space direction="vertical" style={{ width: "100%" }}>
                <Card bordered={false} >
                    <p>在这里，你可以对系统中的用户进行管理，例如添加一个新用户，或者修改系统中已经存在的用户。</p>
                </Card>

...
const Login: React.FC<LoginProps> = props => {
    ...
    const [captchaImg, setCaptchaImg] = useState<Interface.Captcha>({ text: "", img: "" });
    useEffect(() => {
        fetchCaptcha()
    }, []);
    const fetchCaptcha = async () => {
        const CaptchaImg: Interface.Captcha = await accountCaptcha().then(res => { return res })
        ...
        return (<>
            ...
            <Form name="basic" onFinish={handleSubmit} initialValues={{}} form={loginForm}>
                ...
                <Form.Item
                    ...
                    name={[name, 'password']}
                >
                    <Input.Password placeholder={'密码：123456'} prefix={<UnlockOutlined />} />
                </Form.Item>

                {
                    ...
                    >
                        <div className={styles['login-flex']}>
                            ...
                            <div className={styles['captcha-svg']} dangerouslySetInnerHTML={{ __html: captchaImg.img }} onClick={fetchCaptcha}></div>
                        </div>
                    </Form.Item> : <Form.Item
                        label=""

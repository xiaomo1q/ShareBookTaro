...
const RightTop: React.FC<RightTopProps> = ({
  ...
  const toggle = () => {
    onToggleHandler && onToggleHandler(!collapsed);
  };
  const handleClick = (item: any) => {
    onMenuClickHandler && onMenuClickHandler(item);
  };
  // 登出
  ...
  const menu = (
    <Menu>
      {/* <Menu.Item key="0">
            ...
            <Menu.Divider /> */}
      <Menu.Item key="3" onClick={logoutClickedHandler}>
        退出登录
      </Menu.Item>
    </Menu>
  );

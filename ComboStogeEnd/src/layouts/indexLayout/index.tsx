...
const IndexLayout: React.FC<any> = props => {
    ...
    const [sidlerSelectedKeys, setSidlerSelectedKeys] = useState<string>(pathname)
    ...
    const fetchRoute = async (newArr: string, paName: any[string]) => {
        ...
        let itemFilter1 = getPermissionMenuData([JSON.parse(role)], IndexLayoutRoutes)
        let itemFilter = itemFilter1.filter((a: { path: string; }) => a.path === newArr)
        if (itemFilter[0]) {
    ...
    return (
        <Layout id={styles['indexLayout']}>
            <Sider trigger={null} collapsible collapsed={collapsed}>
                ...
                <Left sidlerMenu={sidlerMenu} selectedKey={props.location.pathname} roles={roles} />
            </Sider>
            <Layout className={styles['site-layout']}>
                <Header className={styles['site-layout-top']}>
                    <RightTop collapsed={collapsed} onToggleHandler={toggle} selectedKey={selectedKeys} onMenuClickHandler={menuClickHandler} />
                    <div className={styles['bread-crumb']}><BreadCrumbs list={breadcrumbRoutes} /></div>
                </Header>
